...
app.get('/', function (req, res) {
  res.sendfile(__dirname + '/index-mjp-streamer.html');
});

app.get('/index-mjp-streamer.html', function (req, res) {
  res.sendfile(__dirname + '/index-mjp-streamer.html');
});
...
io.on('connection', function (socket) {
  ...
  socket.on('robot update', function (data) {
    ...
    updatedData['Time'] = new Date();
  	...
  	socket.broadcast.emit('user aaaaaaaa', { 'data': updatedData });
  });
  ...
  socket.on('disconnect', function(){
   ...
   socket.on('robot update', function(){
        console.log( "statussssss");
        socket.broadcast.emit('user aaaaaaaa', { 'data': 'coucou' });
    });
});
...
function GestionCommandes (command) {
  ...
  if (direction!="servo" && direction!="recofaciale") {
    ...
    io.on('connection', function (socket) {
  socket.broadcast.emit('user aaaaaaaa');
});
  } else if (direction=="recofaciale") {
    
	//setTimeout(Prendre_Photo(), 5000);
	//RecoFaciale(imagereco);
  } else if (direction=="servo"){
    DirectionChange(direction, vitesse);
    
  } else if (direction=="robot update"){
    console.log('ROBOT UPDATE');
...
function Prendre_Photo() {
	...
	var options = {
	  mode: 'text',
	  pythonOptions: ['-u'],
	  scriptPath: '/home/pi/projets/drone1/py'
	};
 
		var pyshell = new PythonShell('snap.py', options);
		...
		pyshell.end(function (err) {
		  ...
		  console.log('finished - next step ->');
		  RecoFaciale(imagereco);
		});
	reco = 1;
	//return imagereco;
}
function RecoFaciale(imagereco) {

	console.log('reco faciale dynamique');
	var options = {
	  mode: 'text',
	  pythonOptions: ['-u'],
	  scriptPath: '/home/pi/projets/drone1/py',
	...
	var pyshell = new PythonShell('recofacial-test.py', options);
...
function TestComm() {
	...
	var options = {
	  ...
	  scriptPath: '/home/pi/projets/drone1/py'
		...
		var pyshell = new PythonShell('test-comm-py-node.py', options);
pyshell.send('Recu 5 sur 5 -> jai fais le job');
		pyshell.on('message', function (message) {
		  // received a message sent from the Python script (a simple "print" statement)
		  
		  console.log("Message recu par node ->"+message);
